...
class GameTableRow {
    ...
    getSportsCultAwayTeamToken() {
        return this.getTeamCityLabel(this.game?.awayTeam);
    }

    getSportsCultSearchUrl() {
        const seasonYear = this.getSportsCultSeasonYear();
        const awayTeamToken = this.getSportsCultAwayTeamToken();

        if (!seasonYear || !awayTeamToken) {
            return null;
        }

        const encodedAwayToken = encodeURIComponent(awayTeamToken).replace(/%20/g, '+');
        const searchQuery = `NBA+RS+${seasonYear}+${encodedAwayToken}`;
